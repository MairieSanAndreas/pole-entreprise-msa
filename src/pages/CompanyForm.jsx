  ...
  const submit = async (e) => {
    ...
    try {
      const payload = {
        ...
        coowner2_first_name: form.coowner2_first_name || null,
        coowner2_last_name: form.coowner2_last_name || null,
        coowner2_phone: form.coowner2_phone || null,
  ...
  return (
    <form className="stack" onSubmit={submit} style={{ maxWidth: 820 }}>
      ...
      <div className="card pad">
            ...
            <div className="grid g-2">
              ...
              <Field label="Statut">
                <select value={form.status} onChange={set("status")}>
                  {COMPANY_STATUS.map((s) => <option key={s.value} value={s.value}>{s.label}</option>)}
                </select>
              </Field>
            </div>
          </div>
        ...
        <h3 style={{ fontFamily: "var(--serif)", fontSize: 15, margin: "16px 0 12px" }}>Deuxième copatron <span className="faint" style={{ fontWeight: 400, fontSize: 12 }}>(facultatif)</span></h3>
        <div className="grid g-3">
          <Field label="Prénom"><input value={form.coowner2_first_name} onChange={set("coowner2_first_name")} /></Field>
          <Field label="Nom"><input value={form.coowner2_last_name} onChange={set("coowner2_last_name")} /></Field>
          <Field label="Téléphone"><input value={form.coowner2_phone} onChange={set("coowner2_phone")} /></Field>
        </div>
        ...
        <div className="grid g-2">
          ...
          <Field label="Taxe d'exploitation">
            <input value={form.exploitation_tax} onChange={set("exploitation_tax")} placeholder="ex. 5 000 $ / semaine" />
          </Field>
        </div>
